...
The ``backbone`` parameter is kept in all public signatures for backward
compatibility with older configs/checkpoints (ablation study).
"""
...
def format_caption_prompt(caption: str, backbone: str = "tinyllava") -> str:
    """Format an LCS-558K caption sample."""
    backbone = backbone.lower()
    if backbone != "tinyllava":
        raise ValueError(f"Unknown backbone '{backbone}'. Only 'tinyllava' is supported.")
    return (
...
def format_vqa_prompt(question: str, answer: str, backbone: str = "tinyllava") -> tuple[str, str]:
    ...
    ``prefix='USER: {q} ASSISTANT:'`` (no trailing space),
...
def format_vqa_query(question: str, backbone: str = "tinyllava") -> str:
